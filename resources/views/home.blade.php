        ...
        @else
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @foreach($books as $book)
                    <a href="#" class="bg-white rounded shadow p-3 hover:shadow-md block">
                        <img src="{{ $book->cover_image_path ? asset('storage/' . $book->cover_image_path) : 'https://placehold.co/300x420' }}"
                            alt="{{ $book->title }}" class="w-full h-64 md:h-72 object-cover rounded">
