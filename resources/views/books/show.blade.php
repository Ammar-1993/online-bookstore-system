...
<body class="bg-gray-50 text-gray-900">
  ...
  <main class="container mx-auto px-4 py-8">
    ...
    <div class="grid md:grid-cols-2 gap-8">
      {{-- Cover (fixed aspect ratio 3:4) --}}
      <div class="max-w-xl w-full mx-auto">
        <figure class="rounded-xl shadow bg-white overflow-hidden">
          {{-- إطار بنسبة 3:4: ارتفاع = 133.333% من العرض --}}
          <div class="relative w-full" style="padding-top:133.3333%;">
            <img
              src="{{ $coverSrc }}"
              alt="غلاف: {{ $book->title }}"
              loading="lazy" decoding="async"
              class="absolute inset-0 h-full w-full object-cover"
              onerror="this.onerror=null;this.src='https://placehold.co/600x800';">
          </div>
        </figure>
      </div>
      ...
      <div>
        <div class="flex items-center gap-3 mb-2">
          ...
          @if($count > 0)
            <div class="flex items-center gap-1">
              <div class="flex">
                @for($i = 1; $i <= 5; $i++)
                  <svg class="w-5 h-5 {{ $i <= $avgRounded ? 'text-yellow-400' : 'text-gray-300' }}" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        <div class="flex flex-wrap gap-2 mb-4">
          @if($book->category)
            <a class="px-2 py-1 bg-gray-100 rounded hover:bg-gray-200" href="{{ route('categories.show', $book->category) }}">#{{ $book->category->name }}</a>
          @endif
          @if($book->publisher)
            <a class="px-2 py-1 bg-gray-100 rounded hover:bg-gray-200" href="{{ route('publishers.show', $book->publisher) }}">#{{ $book->publisher->name }}</a>
          @endif
        </div>
        ...
        <div class="text-sm text-gray-600 mb-6">
          ...
          @unless($inStock)
            <span class="ml-2 inline-flex items-center px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-600">غير متاح</span>
          @endunless
        </div>
