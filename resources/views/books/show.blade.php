    ...
    <div class="grid md:grid-cols-2 gap-8">
      ...
      <div>
        <img
          ...
          alt="{{ $book->title }}"
          class="w-full rounded shadow bg-white object-cover">
      </div>
      ...
      <div>
        <div class="flex items-center gap-3 mb-2">
          ...
          @if(($ratingsCount ?? 0) > 0)
            <div class="flex items-center gap-1">
              ...
              <span class="text-xs text-gray-600">({{ number_format((float)$avgRating,1) }}) • {{ (int)$ratingsCount }} مراجعة</span>
            </div>
          @endif
        </div>
        ...
        {{-- زر العربة (لاحقًا) --}}
        <div class="mt-6">
          <button class="px-4 py-2 rounded bg-gray-300 text-gray-700 cursor-not-allowed" title="قريبًا">
            أضف إلى العربة
          </button>
        </div>
      </div>
