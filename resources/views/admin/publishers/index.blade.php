...
<form method="GET" action="{{ route('admin.publishers.index') }}" class="mb-4">
  <div class="relative max-w-xl">
    ...
    @if($q)
      <a href="{{ route('admin.publishers.index', ['sort' => $sort, 'dir' => $dir]) }}"
  ...
  <table class="min-w-full text-sm text-gray-900 dark:text-gray-100">
    <thead class="bg-gray-50 dark:bg-gray-800/60 sticky top-0 z-10">
      <tr class="text-right text-gray-700 dark:text-gray-200">
        <th class="px-3 py-2 w-12 font-medium">{!! $sortLink('id', '#') !!}</th>
        <th class="px-3 py-2 font-medium">{!! $sortLink('name', 'الاسم') !!}</th>
        <th class="px-3 py-2 font-medium">{!! $sortLink('slug', 'Slug') !!}</th>
        <th class="px-3 py-2 font-medium">الموقع</th>
        <th class="px-3 py-2 font-medium text-center">{!! $sortLink('books_count', 'الكتب') !!}</th>
        <th class="px-3 py-2 font-medium text-center">إجراءات</th>
      </tr>
    </thead>
    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
      @forelse($publishers as $publisher)
        <tr class="hover:bg-gray-50/80 dark:hover:bg-white/5">
          <td class="px-3 py-2 tabular-nums text-gray-700 dark:text-gray-300">{{ $publisher->id }}</td>

          <td class="px-3 py-2">
            <a href="{{ route('admin.publishers.edit', $publisher) }}"
               class="font-medium text-gray-900 dark:text-gray-100 hover:underline decoration-indigo-500/50 underline-offset-4">
              {{ $publisher->name }}
            </a>
            @if(!empty($publisher->description))
              <div class="text-xs text-gray-600 dark:text-gray-300 line-clamp-1">{{ $publisher->description }}</div>
            @endif
          </td>

          <td class="px-3 py-2 text-gray-700 dark:text-gray-300"><span dir="ltr" class="break-all">{{ $publisher->slug }}</span></td>

          <td class="px-3 py-2">
            @if($publisher->website)
              <a href="{{ $publisher->website }}" target="_blank" rel="noopener"
                 class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:underline" dir="ltr">
                {{ \Illuminate\Support\Str::limit($publisher->website, 40) }}
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path d="M14 3h7v7M21 3l-9 9" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                  <path d="M21 14v5a2 2 0 0 1-2 2h-5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                </svg>
              </a>
            @else
          ...
          <td class="px-3 py-2">
            <div class="flex items-center justify-center gap-2">
              ...
              <form method="POST" action="{{ route('admin.publishers.destroy', $publisher) }}"
                    onsubmit="return confirm('تأكيد حذف «{{ $publisher->name }}»؟');" class="inline">
                @csrf @method('DELETE')
                <button type="submit"
      ...
      @empty
        <tr>
          <td colspan="6" class="px-3 py-8 text-center text-gray-600 dark:text-gray-300">لا توجد نتائج مطابقة.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
